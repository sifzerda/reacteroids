...
import { settings, setControlScheme } from '../ecs/core/settings.js';
...
export default function SettingsScreen({ onBack }) {
  ...
  const items = [
    { label: `${controlScheme === 'keyboard' ? '☑' : '☐'} KEYBOARD ONLY`, action: () => selectControlScheme('keyboard') },
    { label: `${controlScheme === 'keyboardMouse' ? '☑' : '☐'} MOUSE + KEYBOARD`, action: () => selectControlScheme('keyboardMouse') },
        ...
        {items.map((item, i) => {
          ...
          return (
            ...
            <button key={item.label} onClick={item.action} onMouseEnter={() => setSelected(i)} onMouseLeave={() => setSelected(null)}
              ...
              {item.label}
              {active && (<span className="absolute -left-4 top-1/2 -translate-y-1/2 text-cyan-300 animate-pulse">▶</span>)}
            </button>
          );
        })}
